...
import { ArrowLeft, Copy, Loader2, CheckCircle2, AlertCircle, X, ShieldCheck } from "lucide-react";
...
const FOCUSES = [
  { id: "all", label: "Toàn diện" },
  { id: "bug", label: "🐛 Bug & Lỗi" },
...
export function CodeReview() {
  ...
  const review = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${BASE}/api/code-review`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code, lang, focus }), signal: ctrl.signal,
      });
    ...
    <div className="min-h-screen" style={{ background: "#050505", fontFamily: FONT }}>
      ...
      <div className="relative max-w-2xl mx-auto px-5 pt-28 pb-20" style={{ zIndex: 1 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
              ...
              <ShieldCheck size={20} style={{ color: "rgba(255,255,255,0.8)" }} />
            </div>
            <div>
              <h1 className="text-2xl font-black text-white">Code Review AI</h1>
              <p className="text-sm" style={{ color: "rgba(255,255,255,0.38)" }}>Phân tích code: bug, bảo mật, hiệu năng và gợi ý refactor</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-4">
          <AnimBorderCard speed={7} color="rgba(255,255,255,0.4)" radius={14} innerStyle={{ padding: "0.75rem 1rem" }}>
            <label className="text-xs font-semibold block mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>Tập trung review</label>
            <div className="flex flex-wrap gap-1.5">
              {FOCUSES.map(f => (
                <button key={f.id} onClick={() => setFocus(f.id)}
                  className="px-3 py-1 rounded-lg text-xs font-medium transition-all border"
                  style={{ background: focus === f.id ? "rgba(255,255,255,0.12)" : "rgba(255,255,255,0.04)", borderColor: focus === f.id ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.08)", color: focus === f.id ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.4)" }}>
                  {f.label}
                </button>
              ))}
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.13 }} className="mb-4">
          <AnimBorderCard speed={5} color="rgba(255,255,255,0.6)" radius={16} innerStyle={{ padding: "1rem" }}>
            <textarea value={code} onChange={e => setCode(e.target.value)} placeholder="Dán code cần review vào đây..." rows={14}
        ...
        <motion.button initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.17 }}
          onClick={review} disabled={!code.trim() || loading}
          ...
          {loading ? <><Loader2 size={15} className="animate-spin" /> Đang review...</> : <><ShieldCheck size={15} /> Review code</>}
        </motion.button>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <AnimBorderCard speed={4} color="rgba(255,255,255,0.75)" radius={18} innerStyle={{ padding: "1.25rem" }}>
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold" style={{ color: "rgba(255,255,255,0.4)" }}>Kết quả review</span>
                {result && (
              ...
              {loading && !result && <div className="flex items-center gap-2 text-sm py-4" style={{ color: "rgba(255,255,255,0.35)" }}><Loader2 size={14} className="animate-spin" /> Đang phân tích code...</div>}
